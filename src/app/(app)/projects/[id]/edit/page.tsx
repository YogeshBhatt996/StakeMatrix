...
export default function EditProjectPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      // Support both old single `shift` and new `shifts` array
      if (Array.isArray(data.shifts) && data.shifts.length > 0) {
        setShifts(data.shifts as Shift[]);
      } else if (data.shift) {
        setShifts([data.shift as Shift]);
      }
      setMeetingFreq(data.meetingFrequency as Freq);
      setIsNX(data.isNXProject);
      setIsALIS(data.isALISProject ?? false);
      setIsOther(data.isOtherProject ?? false);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      for (const proc of removedProcesses) {
        await fetch(`/api/projects/${projectId}/processes/${proc.id}`, { method: "DELETE" });
      }
    ...
    <div className="max-w-2xl mx-auto">
      <div className="mb-6">
        <div className="flex items-center gap-2 text-sm text-slate-400 mb-1">
          ...
          <span>/</span>
          <Link href={`/projects/${projectId}`} className="hover:text-slate-600">{projectName}</Link>
          <span>/</span>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-6 space-y-5">
          ...
          <div>
            ...
            <input
              type="date" value={initiationDate}
              ...
              {processes.map((p, i) => (
                <span key={p.id ?? `new-${i}`}
                  ...
                  <button type="button" onClick={() => removeProcess(i)}
                    className="text-indigo-400 hover:text-indigo-700 transition-colors" aria-label={`Remove ${p.name}`}>
